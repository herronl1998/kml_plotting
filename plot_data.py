...
ax.legend()
plt.title(f'Above vs Below {round(m_to_f(alt_thresh))}ft')
...
ax.add_feature(cfeature.LAND)
ax.gridlines(draw_labels = True)
ax.scatter(above_10k['lon'], above_10k['lat'], c = 'green', s = marker_size, label = f'> {round(m_to_f(alt_thresh))}ft')
ax.legend()
fig.title(f'Above {round(m_to_f(alt_thresh))}ft')
...
ax.add_feature(cfeature.LAND)
ax.gridlines(draw_labels = True)
ax.scatter(below_10k['lon'], below_10k['lat'], c = 'red', s = marker_size, label = f'<= {round(m_to_f(alt_thresh))}ft')
ax.legend()
fig.title(f'Below {round(m_to_f(alt_thresh))}ft')
